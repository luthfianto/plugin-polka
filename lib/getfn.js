...
module.exports = function (merapi) {
    return async(function* (str) {
        const [comName, fns] = str.split(".");
        const com = yield merapi.resolve(comName);

        const foundIndex = fns.search(/\(.*\)/);
        let fnName = fns;
        let params;
        ...
        if (com && com[fnName]) {
            const func = Array.isArray(params) ? com[fnName].bind(com, ...params) : com[fnName].bind(com);
            if (func.length === 4) {
